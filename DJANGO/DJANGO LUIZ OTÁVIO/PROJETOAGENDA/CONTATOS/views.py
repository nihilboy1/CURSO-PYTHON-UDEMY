...
def busca(request):
    try:
        ...
        if termo is None or not termo:
            messages.add_message(request, messages.ERROR, 'Pesquisa inválida!')
            return redirect('index')
        
        campos = Concat('nome', Value(' '), 'sobrenome')
        contatos = Contato.objects.annotate(nome_completo=campos).filter(nome_completo__icontains = termo)
